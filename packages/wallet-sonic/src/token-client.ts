/**
 * EIP-3009 token client for Sonic Blaze testnet — read balance + build/sign/
 * submit transferWithAuthorization.
 *
 * Same three-step flow as the Base Sepolia reference impl:
 ...
 *
 * Compatible with any ERC20 implementing EIP-3009 (USDC-shaped). The Sonic VM
 * is fully EVM-equivalent, so the signing path is byte-for-byte identical to
 * other EVM chains; only chainId (57054) + verifyingContract differ.
 *
 * @license Apache-2.0
 */
...
import { sonicBlazeChain } from "./chain.js";
...
export interface Eip3009SignedAuthorization {
  ...
  readonly r: Hex;
  readonly s: Hex;
  /** The token's chainId (57054 for Sonic Blaze testnet). */
  readonly chainId: number;
  /** The token contract address that the signature is bound to. */
  readonly verifyingContract: Address;
}

export interface SonicTokenClientConfig {
  /** Token contract (USDC-shaped EIP-3009 ERC-20 on Sonic Blaze). */
  ...
  /**
   * Optional override for the EIP-712 domain `name`. When set, the signer skips
   * the on-chain `name()` read (handy for offline signing against tokens whose
   * RPC is unavailable). Defaults to reading `name()` from the contract.
   */
 ...
 * transferWithAuthorization on Sonic Blaze testnet.
 */
export class SonicTokenClient {
  ...
  constructor(config: SonicTokenClientConfig) {
  ...
  async getName(): Promise<string> {
    if (this.domainName !== undefined) return this.domainName;
  ...
  // ---- Verification (offline, no RPC) -------------------------------------

  /**
   * Verify an EIP-3009 signature offline by recovering the EIP-712 typed-data
   * signer and checking it equals `authorization.from`. Pure crypto — no chain
   * I/O — so it works for tampered-fails tests and conformance.
   */
  async verifySignedAuthorization(
    ...
    return await verifyTypedData({
      address: signed.authorization.from,
      domain: {
        name: expectedName,
        version: this.domainVersion,
