/**
 * TRON MAINNET wallet ↔ Conformance suite — proves TronMainnetConnector backed
 * by the RealTronMainnetSigner passes the canonical 25-test WalletConnector
 * contract.
 *
...
runWalletConformance(
  ...
  {
    createConnector: () =>
      new TronMainnetConnector({
        signer: new RealTronMainnetSigner({
          privateKeyHex: TEST_PRIVKEY,
          network: "mainnet",
        }),
        instrumentStore: new MemoryInstrumentStore(),
      }),
    createUserId: (suffix) => `tron-mainnet-test-${suffix}` as UserId,
  ...
  },
  {
    ...
    skipSettle: false,
    suiteName: "wallet-tron-mainnet conformance (RealTronMainnetSigner)",
  }
);
