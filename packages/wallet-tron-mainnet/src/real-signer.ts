/**
 * RealTronMainnetSigner — secp256k1 (ECDSA) signer backed by @noble/curves, no tronweb.
 * ============================================================================
 *
 * MAINNET variant of the TRON signer. Identical crypto to the testnet signer
 * (the 0x41 prefix byte is the same on mainnet + testnets, so all addresses
 * begin with "T"); only the default network and explorer host differ.
 *
 ...
 * touching this file. MAINNET broadcast moves real value — guard it accordingly.
 *
 * @license Apache-2.0
 */
...
/** TRON address prefix byte (0x41 → all addresses start "T", mainnet + testnets). */
 ...
 * identical in shape to a real funded mainnet account.
 */
...
export interface RealTronMainnetSignerConfig {
   ...
   * On MAINNET this moves real value; wire it only behind explicit opt-in.
   */
  ...
  /** Network for explorer URLs (defaults to mainnet for this signer). */
...
export class RealTronMainnetSigner {
  readonly address: string;
  readonly addressHex: string;
  private readonly priv: Uint8Array;
  private readonly cfg: RealTronMainnetSignerConfig;
  private readonly network: "mainnet" | "shasta" | "nile";

  constructor(cfg: RealTronMainnetSignerConfig = {}) {
    let kp: TronKeypair;
    if (cfg.privateKey) {
