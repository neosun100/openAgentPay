/**
 * EIP-3009 token client for Berachain bArtio — read balance + build / sign /
 ...
 *
 * Compatible with any ERC20 implementing EIP-3009 (USDC-shaped) on the
 * EVM-identical Berachain bArtio execution layer.
 *
 * @license Apache-2.0
 */
...
import { berachainBartioChain } from "./chain.js";
...
export interface Eip3009SignedAuthorization {
  ...
  readonly r: Hex;
  readonly s: Hex;
  /** The token's chainId (80084 for Berachain bArtio). */
  readonly chainId: number;
  /** The token contract address that the signature is bound to. */
  readonly verifyingContract: Address;
}

export interface BerachainTokenClientConfig {
  /** Token contract (e.g. USDC-style stablecoin on Berachain bArtio). */
 ...
 * transferWithAuthorization on Berachain bArtio.
 */
export class BerachainTokenClient {
  public readonly chain: Chain;
  public readonly tokenAddress: Address;
  public readonly publicClient: PublicClient;
  private readonly domainVersion: string;

  constructor(config: BerachainTokenClientConfig) {
    this.chain = config.chain ?? berachainBartioChain;
    this.tokenAddress = config.tokenAddress;
    this.domainVersion = config.domainVersion ?? "2";
    const rpc = config.rpcUrl ?? this.chain.rpcUrls.default.http[0];
    this.publicClient =
      config.publicClient ??
      createPublicClient({
        chain: this.chain,
   ...
   *
   * `tokenName` may be passed to avoid a network read (used in offline tests);
   * otherwise the EIP-712 domain `name` is fetched from the token contract.
   */
