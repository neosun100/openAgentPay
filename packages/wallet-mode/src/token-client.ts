/**
 * EIP-3009 token client for Mode Sepolia — read balance + build/sign/submit
 ...
 *
 * Compatible with any ERC20 implementing EIP-3009 (USDC-shaped) on Mode Sepolia.
 *
 * @license Apache-2.0
 */
...
import { modeSepoliaChain } from "./chain.js";
...
export interface Eip3009SignedAuthorization {
  ...
  readonly r: Hex;
  readonly s: Hex;
  /** The token's chainId (919 for Mode Sepolia). */
  readonly chainId: number;
  /** The token contract address that the signature is bound to. */
  readonly verifyingContract: Address;
}

export interface ModeTokenClientConfig {
  /** Token contract (EIP-3009 USDC on Mode Sepolia). */
 ...
 * transferWithAuthorization on Mode Sepolia.
 */
export class ModeTokenClient {
  public readonly chain: Chain;
  public readonly tokenAddress: Address;
  public readonly publicClient: PublicClient;
  private readonly domainVersion: string;

  constructor(config: ModeTokenClientConfig) {
  ...
  /**
   * Verify an EIP-3009 signed authorization off-chain (no RPC). Recovers the
   * signer from the EIP-712 typed-data hash and checks it equals
   * `authorization.from`. This is the local-verifiable check used by the
   * connector's signAuthorization round-trip test.
   */
  async verifyTransferAuthorization(
