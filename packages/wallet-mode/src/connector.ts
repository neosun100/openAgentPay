/**
 * ModeSepoliaConnector — implements WalletConnector for self-custodial EVM
 * wallets on Mode Sepolia (Mode's OP-Stack L2 testnet, chainId 919).
 *
 ...
 *
 * Asset: USDC on Mode Sepolia (6 decimals, placeholder address). Protocol: x402-v1.
 *
 * @license Apache-2.0
 */
...
export const WALLET_PROVIDER_ID = "mode" as WalletProviderId;

/** OpenAgentPay uses `x402-v1` as the canonical protocol id for EIP-3009 flows. */
export const MODE_PROTOCOL: ProtocolId = "x402-v1" as ProtocolId;

/** CAIP-2 chain id for Mode Sepolia. */
export const MODE_CAIP2 = "eip155:919" as const;

// ============================================================================
//  Configuration
// ============================================================================

export interface ModeSepoliaConnectorConfig {
  /**
  ...
  /** Token contract address. Default: placeholder USDC on Mode Sepolia. */
  readonly tokenAddress?: Address;
  /** Chain to operate on. Default: Mode Sepolia. */
  ...
  readonly tokenClient?: ModeTokenClient;
}
...
const SUPPORTED_ASSETS: readonly Asset[] = [
  { symbol: "USDC", decimals: 6, chain: MODE_CAIP2, contract: MODE_SEPOLIA_USDC },
];

export class ModeSepoliaConnector implements WalletConnector {
  private readonly tokenClient: ModeTokenClient;
  ...
  constructor(config: ModeSepoliaConnectorConfig) {
    this.chain = config.chain ?? modeSepoliaChain;
    this.now = config.now ?? Date.now;
    this.store = config.instrumentStore;

    const tokenAddress = config.tokenAddress ?? MODE_SEPOLIA_USDC;
  ...
  getCapabilities(): WalletCapabilities {
    return {
      ...
      displayName: "Mode Sepolia (Self-Custodial)",
      supportedAssets: SUPPORTED_ASSETS,
      supportedProtocols: [MODE_PROTOCOL],
      requiresUserApproval: false, // private key in Secrets Manager → no UI prompt
      settlesOnChain: true,
      typicalLatencyMs: 2000, // ~2s block time on Mode L2
      features: {
  ...
  async createInstrument(input: CreateInstrumentInput): Promise<Instrument> {
    ...
    const id = `payment-instrument-mode-${input.userId}` as InstrumentId;
  ...
  async signAuthorization(
    ...
    if (input.request.protocol !== MODE_PROTOCOL) {
      throw new Error(
        `ModeSepoliaConnector only supports protocol ${MODE_PROTOCOL}, got ${input.request.protocol}`
      );
    }
