/**
 * OptimismConnector — implements WalletConnector for self-custodial EVM
 * wallets on Optimism (OP Sepolia testnet, chainId 11155420).
 *
 * Self-custodial model (mirrors packages/wallet-hashkey):
 *   - The agent (or its operator) holds the EOA private key
 *   - In production the key lives in AWS Secrets Manager + KMS
 *   - The connector loads it just-in-time, signs EIP-712, returns a
 ...
 *     wallet (which can be the same EOA in demo, or a separate gas-paying EOA
 *     in production)
 *
 * If no privateKey is supplied, a fresh random EVM keypair is generated
 * in-process (useful for ephemeral demo agents / tests).
 *
 * @license Apache-2.0
 */
...
import { optimismSepoliaChain, OP_SEPOLIA_USDC, txExplorerUrl } from "./chain.js";
import {
  OptimismTokenClient,
...
export const WALLET_PROVIDER_ID = "optimism" as WalletProviderId;

/** OpenAgentPay uses `x402-v1` as the canonical protocol id for EIP-3009 flows. */
export const OPTIMISM_PROTOCOL: ProtocolId = "x402-v1" as ProtocolId;

// ============================================================================
//  Configuration
// ============================================================================

export interface OptimismConnectorConfig {
  /**
   * The agent's private key (Hex, 0x-prefixed). Optional — if omitted, a fresh
   * random keypair is generated in-process. In production load from AWS
   * Secrets Manager.
   */
  readonly privateKey?: Hex;
  /** Token contract address. Default: Circle USDC on OP Sepolia. */
  readonly tokenAddress?: Address;
  /** Chain to operate on. Default: OP Sepolia. */
  readonly chain?: typeof optimismSepoliaChain;
  ...
  /**
   * Optional separate facilitator wallet for broadcasting (gas-payer). If
   * omitted, the agent's own wallet pays gas + broadcasts (simplest demo).
   */
  readonly facilitatorPrivateKey?: Hex;
  /** Optional clock — overridable in tests. */
  readonly now?: () => number;
  /** Optional override for token client (for testing). */
  readonly tokenClient?: OptimismTokenClient;
}
...
export class OptimismConnector implements WalletConnector {
  private readonly tokenClient: OptimismTokenClient;
  ...
  private readonly chain: typeof optimismSepoliaChain;
  private readonly now: () => number;

  constructor(private readonly config: OptimismConnectorConfig) {
    this.chain = config.chain ?? optimismSepoliaChain;
    this.now = config.now ?? Date.now;
    this.store = config.instrumentStore;

    const tokenAddress = config.tokenAddress ?? (OP_SEPOLIA_USDC as Address);
    const privateKey = config.privateKey ?? generateEvmPrivateKey();
  ...
  getCapabilities(): WalletCapabilities {
    return {
      ...
      displayName: "Optimism (OP Sepolia, Self-Custodial)",
      supportedAssets: SUPPORTED_ASSETS,
      supportedProtocols: [OPTIMISM_PROTOCOL],
      requiresUserApproval: false, // private key in Secrets Manager → no UI prompt
      settlesOnChain: true,
      typicalLatencyMs: 2500, // empirical: ~2.5s on OP Sepolia (2s block time)
      features: {
        gasInNativeToken: true,
        instantFinality: false, // L2 optimistic rollup
  ...
  async createInstrument(input: CreateInstrumentInput): Promise<Instrument> {
    ...
    const id = `payment-instrument-optimism-${input.userId}` as InstrumentId;
  ...
  async signAuthorization(
    ...
    if (input.request.protocol !== OPTIMISM_PROTOCOL) {
      throw new Error(
        `OptimismConnector only supports protocol ${OPTIMISM_PROTOCOL}, got ${input.request.protocol}`
      );
    }
