/**
 * EIP-3009 token client for OP Sepolia — read balance + build/sign/submit
 * transferWithAuthorization.
 *
 * Same flow as packages/wallet-hashkey (the canonical EVM connector):
 ...
 *
 * Compatible with any ERC20 implementing EIP-3009 — here, Circle USDC on
...
import { optimismSepoliaChain } from "./chain.js";
...
export interface Eip3009SignedAuthorization {
  ...
  readonly r: Hex;
  readonly s: Hex;
  /** The token's chainId (11155420 for OP Sepolia). */
  readonly chainId: number;
  /** The token contract address that the signature is bound to. */
  readonly verifyingContract: Address;
}

export interface OptimismTokenClientConfig {
  /** Token contract (Circle USDC on OP Sepolia). */
  ...
  /** EIP-712 domain version of the token. Circle USDC uses "2". */
 ...
 * transferWithAuthorization on OP Sepolia.
 */
export class OptimismTokenClient {
  public readonly chain: typeof optimismSepoliaChain;
  public readonly tokenAddress: Address;
  public readonly publicClient: PublicClient;
  private readonly domainVersion: string;

  constructor(config: OptimismTokenClientConfig) {
  ...
  /**
   * Sign an EIP-3009 transferWithAuthorization off-chain. The signer address
   * MUST match `authorization.from`, otherwise on-chain `ecrecover` rejects it.
   */
  ...
  /** Wait for a tx to be mined and report status. */
...
/** Generate a fresh random EVM private key (0x-prefixed Hex). */
export function generateEvmPrivateKey(): Hex {
...
export function createWalletClientFromPrivateKey(
  privateKey: Hex,
  chain: typeof optimismSepoliaChain,
  rpcUrl?: string
