/**
 * Base wallet ↔ Conformance suite — proves the connector passes the canonical
...
runWalletConformance(
  ...
  {
    createConnector: () =>
      new BaseSepoliaConnector({
        privateKey: TEST_PRIVATE_KEY,
        tokenAddress: TEST_TOKEN,
        instrumentStore: new MemoryInstrumentStore(),
        tokenClient: makeStubTokenClient(),
      }),
    createUserId: (suffix) => `base-test-${suffix}` as UserId,
  ...
  },
  {
    ...
    skipSettle: false,
    suiteName: "wallet-base conformance",
  }
);
