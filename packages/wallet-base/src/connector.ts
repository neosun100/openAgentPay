/**
 * BaseSepoliaConnector — implements WalletConnector for self-custodial EVM
 * wallets on Base Sepolia (Coinbase's OP-Stack L2 testnet, chainId 84532).
 *
 * Self-custodial model (same as wallet-hashkey):
 ...
 *
 * Asset: Circle USDC on Base Sepolia (6 decimals). Protocol: x402-v1.
 *
 * @license Apache-2.0
 */
...
export const WALLET_PROVIDER_ID = "base-sepolia" as WalletProviderId;

/** OpenAgentPay uses `x402-v1` as the canonical protocol id for EIP-3009 flows. */
export const BASE_PROTOCOL: ProtocolId = "x402-v1" as ProtocolId;

// ============================================================================
//  Configuration
// ============================================================================

export interface BaseSepoliaConnectorConfig {
  /**
  ...
  /** Token contract address. Default: Circle USDC on Base Sepolia. */
  readonly tokenAddress?: Address;
  /** Chain to operate on. Default: Base Sepolia. */
  ...
  readonly tokenClient?: BaseTokenClient;
}
...
export class BaseSepoliaConnector implements WalletConnector {
  private readonly tokenClient: BaseTokenClient;
  ...
  constructor(config: BaseSepoliaConnectorConfig) {
    this.chain = config.chain ?? baseSepoliaChain;
    this.now = config.now ?? Date.now;
    this.store = config.instrumentStore;

    const tokenAddress = config.tokenAddress ?? BASE_SEPOLIA_USDC;
  ...
  getCapabilities(): WalletCapabilities {
    return {
      ...
      displayName: "Base Sepolia (Self-Custodial)",
      supportedAssets: SUPPORTED_ASSETS,
      supportedProtocols: [BASE_PROTOCOL],
      requiresUserApproval: false, // private key in Secrets Manager → no UI prompt
      settlesOnChain: true,
      typicalLatencyMs: 2000, // ~2s block time on Base L2
      features: {
  ...
  async createInstrument(input: CreateInstrumentInput): Promise<Instrument> {
    ...
    const id = `payment-instrument-base-${input.userId}` as InstrumentId;
  ...
  async signAuthorization(
    ...
    if (input.request.protocol !== BASE_PROTOCOL) {
      throw new Error(
        `BaseSepoliaConnector only supports protocol ${BASE_PROTOCOL}, got ${input.request.protocol}`
      );
    }
