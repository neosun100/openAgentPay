/**
 * EIP-3009 token client for Base Sepolia — read balance + build/sign/submit
 * transferWithAuthorization.
 *
 * Same three-step flow as the HashKey reference impl:
 ...
 *
 * Compatible with any ERC20 implementing EIP-3009, including Circle USDC on
 * Base Sepolia (the default asset for this connector).
 *
 * @license Apache-2.0
 */
...
import { baseSepoliaChain } from "./chain.js";
...
export interface Eip3009SignedAuthorization {
  ...
  readonly r: Hex;
  readonly s: Hex;
  /** The token's chainId (84532 for Base Sepolia). */
  readonly chainId: number;
  /** The token contract address that the signature is bound to. */
  readonly verifyingContract: Address;
}

export interface BaseTokenClientConfig {
  /** Token contract (e.g. Circle USDC on Base Sepolia). */
 ...
 * transferWithAuthorization on Base Sepolia.
 */
export class BaseTokenClient {
  public readonly chain: Chain;
  public readonly tokenAddress: Address;
  public readonly publicClient: PublicClient;
  private readonly domainVersion: string;

  constructor(config: BaseTokenClientConfig) {
