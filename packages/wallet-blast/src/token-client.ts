/**
 * EIP-3009 token client for Blast Sepolia — read balance + build/sign/submit
 * transferWithAuthorization.
 *
 * Same three-step flow as the Base reference impl:
 ...
 *
 * Compatible with any ERC20 implementing EIP-3009 on Blast Sepolia.
 *
 * @license Apache-2.0
 */
...
import { blastSepoliaChain } from "./chain.js";
...
export interface Eip3009SignedAuthorization {
  ...
  readonly r: Hex;
  readonly s: Hex;
  /** The token's chainId (168587773 for Blast Sepolia). */
  readonly chainId: number;
  /** The token contract address that the signature is bound to. */
  readonly verifyingContract: Address;
}

export interface BlastTokenClientConfig {
  /** Token contract (e.g. USDC on Blast Sepolia). */
 ...
 * transferWithAuthorization on Blast Sepolia.
 */
export class BlastTokenClient {
  public readonly chain: Chain;
  public readonly tokenAddress: Address;
  public readonly publicClient: PublicClient;
  private readonly domainVersion: string;

  constructor(config: BlastTokenClientConfig) {
