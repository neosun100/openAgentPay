/**
 * ArbitrumConnector — implements WalletConnector for self-custodial EVM
 * wallets on Arbitrum (Sepolia testnet 421614 / One mainnet 42161).
 *
 * Arbitrum is an Optimistic-Rollup L2 with EVM equivalence, so this connector
 * is a near-verbatim sibling of `@openagentpay/wallet-hashkey`: viem walletClient
 * + EIP-712 EIP-3009 `transferWithAuthorization` signing, settle via a
 * facilitator wallet broadcast.
 *
 ...
 *     in-process (viem generatePrivateKey) — handy for offline conformance
 *     runs and demos.
 *
 * Settlement model:
 ...
 *     wallet (defaults to the same EOA in demo; separate gas-payer in prod)
 *
 * @license Apache-2.0
 */
...
export const WALLET_PROVIDER_ID = "arbitrum" as WalletProviderId;

/** OpenAgentPay uses `x402-v1` as the canonical protocol id for EVM stablecoin flows. */
export const ARBITRUM_PROTOCOL: ProtocolId = "x402-v1" as ProtocolId;

// ============================================================================
//  Configuration
// ============================================================================

export interface ArbitrumConnectorConfig {
  /**
  ...
  /** Token contract address. Default: Circle USDC on Arbitrum Sepolia. */
  readonly tokenAddress?: Address;
  /** Chain to operate on. Default: Arbitrum Sepolia Testnet. */
  readonly chain?: typeof arbitrumSepoliaTestnet;
  ...
  readonly tokenClient?: ArbitrumTokenClient;
}
...
export class ArbitrumConnector implements WalletConnector {
  private readonly tokenClient: ArbitrumTokenClient;
  ...
  private readonly chain: typeof arbitrumSepoliaTestnet;
  private readonly tokenAddress: Address;
  private readonly now: () => number;

  constructor(private readonly config: ArbitrumConnectorConfig) {
    this.chain = config.chain ?? arbitrumSepoliaTestnet;
    this.tokenAddress = config.tokenAddress ?? (ARBITRUM_SEPOLIA_USDC as Address);
  ...
  getCapabilities(): WalletCapabilities {
    return {
      ...
      displayName: "Arbitrum (Self-Custodial)",
      supportedAssets: SUPPORTED_ASSETS,
      supportedProtocols: [ARBITRUM_PROTOCOL],
      requiresUserApproval: false, // private key in Secrets Manager → no UI prompt
      settlesOnChain: true,
      typicalLatencyMs: 1200, // empirical: Arbitrum L2 sub-second-ish confirmation
      features: {
        gasInNativeToken: true,
        instantFinality: false, // optimistic rollup — soft finality fast, hard finality on L1
  ...
  async createInstrument(input: CreateInstrumentInput): Promise<Instrument> {
    ...
    const id = `payment-instrument-arbitrum-${input.userId}` as InstrumentId;
  ...
  async signAuthorization(
    ...
    if (input.request.protocol !== ARBITRUM_PROTOCOL) {
      throw new Error(
        `ArbitrumConnector only supports protocol ${ARBITRUM_PROTOCOL}, got ${input.request.protocol}`
      );
    }
   ...
   * with on-chain tx hash + Arbiscan link in `raw`.
   */
...
// Re-export Money to keep the type surface explicit for downstream consumers.
export type { Money };
