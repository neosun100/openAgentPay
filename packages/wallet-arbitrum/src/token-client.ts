/**
 * EIP-3009 token client for Arbitrum — read balance + build/sign/submit
 * transferWithAuthorization.
 *
 * Same flow as `@openagentpay/wallet-hashkey`'s token-client:
 ...
 *
 * Works against Circle's official USDC on Arbitrum Sepolia (chainId 421614),
 * which implements EIP-3009 with the same typehashes as Circle USDC on Base.
 *
 * @license Apache-2.0
 */
...
import { arbitrumSepoliaTestnet } from "./chain.js";
...
export interface Eip3009SignedAuthorization {
  ...
  readonly r: Hex;
  readonly s: Hex;
  /** The token's chainId (e.g. 421614 for Arbitrum Sepolia). */
  readonly chainId: number;
  /** The token contract address that the signature is bound to. */
  readonly verifyingContract: Address;
}

export interface ArbitrumTokenClientConfig {
  /** Token contract (e.g. Circle USDC at 0x75faf1...). */
  ...
  /**
   * Optional EIP-712 domain version override. Circle USDC uses "2"; this lets
   * us target other EIP-3009 tokens (e.g. "1") without an on-chain read.
   */
 ...
 */
export class ArbitrumTokenClient {
  public readonly chain: typeof arbitrumSepoliaTestnet;
  public readonly tokenAddress: Address;
  public readonly publicClient: PublicClient;
  private readonly domainVersion: string;

  constructor(config: ArbitrumTokenClientConfig) {
...
export function createWalletClientFromPrivateKey(
  privateKey: Hex,
  chain: typeof arbitrumSepoliaTestnet,
  rpcUrl?: string
