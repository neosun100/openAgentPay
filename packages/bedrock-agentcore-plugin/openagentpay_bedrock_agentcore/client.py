...
Mirrors ``openagentpay_pydantic_ai.client`` — does NOT reimplement payment
logic. It just talks the same REST contract the python-sdk uses:

    POST /api/session   create a session with budget cap + TTL
    POST /api/pay       sign + settle (returns tx hash on success)

Private keys never live here — they stay server-side, secured by
@openagentpay/governance (Layer 3+5+7 guardrails).
"""
...
class OpenAgentPayClient:
    ...
    reused for subsequent calls. If a payment 404s (session expired / routed
    to a cold Lambda), the session cache is invalidated and a fresh session
    is created for a one-shot retry.
    """
    ...
    async def pay(
        ...
        *,
        ...
        Returns the raw JSON body from /api/pay. Governance denials and chain
        failures come back as ``{"success": false, ...}`` (NOT exceptions).
        ...
        try:
            ...
            if err.http_status == 404:
                # Session expired across instances → recreate, retry once.
                self._session = _SessionState()
                sess_id = await self._ensure_session()
                return await self._do_pay(
                    session_id=sess_id,
    ...
    async def _ensure_session(self) -> str:
        ...
        data = r.json()
        # demo-api uses sessionId; some deployments use id — accept both.
