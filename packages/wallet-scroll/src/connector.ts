/**
 * ScrollSepoliaConnector — implements WalletConnector for self-custodial EVM
 * wallets on Scroll Sepolia (Scroll's zkEVM L2 testnet, chainId 534351).
 *
 ...
 *
 * Asset: USDC on Scroll Sepolia (6 decimals). Protocol: x402-v1.
 *
 * @license Apache-2.0
 */
...
export const WALLET_PROVIDER_ID = "scroll" as WalletProviderId;

/** OpenAgentPay uses `x402-v1` as the canonical protocol id for EIP-3009 flows. */
export const SCROLL_PROTOCOL: ProtocolId = "x402-v1" as ProtocolId;

/**
 * Default EIP-712 domain name for the placeholder testnet USDC. Lets the
 * signing path run fully offline (no `name()` round-trip) so conformance +
 * unit tests are deterministic. Override via config.domainName.
 */
...
export interface ScrollSepoliaConnectorConfig {
  /**
  ...
  /** Token contract address. Default: USDC placeholder on Scroll Sepolia. */
  readonly tokenAddress?: Address;
  /** Chain to operate on. Default: Scroll Sepolia. */
  ...
  readonly tokenClient?: ScrollTokenClient;
  /** Optional EIP-712 domain name override (defaults to "USD Coin"). */
  readonly domainName?: string;
  /** Optional EIP-712 domain version override (defaults to "2"). */
...
const SUPPORTED_ASSETS: readonly Asset[] = [
  { symbol: "USDC", decimals: 6, chain: "eip155:534351", contract: SCROLL_SEPOLIA_USDC },
];

export class ScrollSepoliaConnector implements WalletConnector {
  private readonly tokenClient: ScrollTokenClient;
  ...
  constructor(config: ScrollSepoliaConnectorConfig) {
    this.chain = config.chain ?? scrollSepoliaChain;
    this.now = config.now ?? Date.now;
    this.store = config.instrumentStore;

    const tokenAddress = config.tokenAddress ?? SCROLL_SEPOLIA_USDC;
    ...
    this.tokenClient =
      config.tokenClient ??
      new ScrollTokenClient({
        tokenAddress,
        chain: this.chain,
        domainName: config.domainName ?? DEFAULT_DOMAIN_NAME,
        domainVersion: config.domainVersion ?? "2",
  ...
  getCapabilities(): WalletCapabilities {
    return {
      ...
      displayName: "Scroll Sepolia (Self-Custodial)",
      supportedAssets: SUPPORTED_ASSETS,
      supportedProtocols: [SCROLL_PROTOCOL],
      requiresUserApproval: false, // private key in Secrets Manager → no UI prompt
      settlesOnChain: true,
      typicalLatencyMs: 3000, // ~3s block time on Scroll zkEVM L2
      features: {
        gasInNativeToken: true,
        instantFinality: false, // L2; zkEVM proof finality is delayed
  ...
  async createInstrument(input: CreateInstrumentInput): Promise<Instrument> {
    ...
    const id = `payment-instrument-scroll-${input.userId}` as InstrumentId;
  ...
  async getBalance(instrumentId: InstrumentId): Promise<Balance> {
    ...
    // Read live balance. The default token is a testnet placeholder that may
    // not be deployed at the placeholder address; fall back to 0 so the
    // balance read returns a valid Money rather than throwing on a missing
    // contract. Override tokenAddress with a real deployment for live reads.
    let decimals = 6;
    let balanceWei = 0n;
    try {
  ...
  async signAuthorization(
    ...
    if (input.request.protocol !== SCROLL_PROTOCOL) {
      throw new Error(
        `ScrollSepoliaConnector only supports protocol ${SCROLL_PROTOCOL}, got ${input.request.protocol}`
      );
    }
  ...
  /** Verify a SignedAuthorization produced by this connector (off-chain). */
  async verify(signed: SignedAuthorization): Promise<boolean> {
    const wireSigned = signed.extra?.["signed"] as
      | Eip3009SignedAuthorization
      | undefined;
    if (!wireSigned) return false;
    return await this.tokenClient.verifySignedAuthorization(wireSigned);
  }
...
export { SCROLL_SEPOLIA_CHAIN_ID };
