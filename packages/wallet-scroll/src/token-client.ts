/**
 * EIP-3009 token client for Scroll Sepolia — read balance + build/sign/submit
 * transferWithAuthorization, plus an off-chain signature verifier.
 *
 * Three-step settlement flow (identical to the Base/HashKey reference impls):
...
import { scrollSepoliaChain } from "./chain.js";
...
export interface Eip3009SignedAuthorization {
  ...
  readonly r: Hex;
  readonly s: Hex;
  /** The token's chainId (534351 for Scroll Sepolia). */
...
export interface ScrollTokenClientConfig {
  /** Token contract (e.g. USDC on Scroll Sepolia). */
  ...
  /**
   * EIP-712 domain `version` string for the token. Circle-style USDC uses "2".
   * Override only if pointing at a token that declares a different version.
   */
  readonly domainVersion?: string;
  /**
   * Optional override for the EIP-712 domain `name`. When omitted the client
   * reads `name()` from the contract on-chain. Supplying it lets the signing
   * path run fully offline (used by conformance + unit tests).
   */
  readonly domainName?: string;
}
...
/**
 * Stateless client for reading EIP-3009 token state, signing / verifying /
 * submitting transferWithAuthorization on Scroll Sepolia.
 */
export class ScrollTokenClient {
  public readonly chain: Chain;
  public readonly tokenAddress: Address;
  public readonly publicClient: PublicClient;
  private readonly domainVersion: string;
  private readonly domainNameOverride: string | undefined;

  constructor(config: ScrollTokenClientConfig) {
    this.chain = config.chain ?? scrollSepoliaChain;
    this.tokenAddress = config.tokenAddress;
    this.domainVersion = config.domainVersion ?? "2";
    this.domainNameOverride = config.domainName;
    this.publicClient =
      config.publicClient ??
      createPublicClient({
        chain: this.chain,
  ...
  async getName(): Promise<string> {
    if (this.domainNameOverride !== undefined) return this.domainNameOverride;
  ...
  /**
   * Verify a signed authorization off-chain by recovering the EIP-712 signer.
   * Mirrors what the token's on-chain `ecrecover` will check. Pure crypto,
   * no network I/O.
   */
