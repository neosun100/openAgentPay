/**
 * TON Pay Protocol Adapter + Wallet Connector — MAINNET build.
 * ============================================================================
 *
 * MAINNET variant of @openagentpay/wallet-ton. Same ton-pay-v1 protocol, same
 * Ed25519 crypto, but defaults target TON MAINNET:
 *
 *   - walletProvider = "ton-mainnet"
 *   - addresses: bounceable mainnet (tag 0x11, workchain 0x00, EQ… prefix)
 *   - network = "mainnet" (explorer at tonviewer.com, settlement label ton-mainnet)
 *
...
export const WALLET_PROVIDER_ID = "ton-mainnet" as WalletProviderId;
...
// "stable" jetton; deployments override via knownJettons.
...
export class DemoTonSigner implements TonSigner {
  ...
  async signAndSubmit(input: {
    ...
    return {
      ...
      txHash: tx,
      explorerUrl: `https://tonviewer.com/transaction/${tx}`,
    };
  }
...
export class TonConnector implements WalletConnector {
  ...
  async createInstrument(input: CreateInstrumentInput): Promise<Instrument> {
    ...
    const id = `payment-instrument-ton-mainnet-${input.userId}` as InstrumentId;
