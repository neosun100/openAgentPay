/**
 * RealTonSigner (MAINNET) — Ed25519 signer for TON (The Open Network).
 * ============================================================================
 *
 * MAINNET variant of the TON connector. Identical cryptography to the
 * testnet build (Ed25519 via @noble/curves, sha256, CRC16-CCITT, base64url),
 * but the *default* address flags target TON MAINNET:
 *
 *   - tag byte 0x11  → bounceable, NO 0x80 test-only bit (testnet uses 0x91)
 *   - workchain 0x00 → basechain
 *   - 48-char base64url user-friendly form, almost always starting with "EQ"
 *     (the EQ prefix comes from the leading byte 0x11 0x00… encoding).
 *
 * The cryptographic identity (keypair → address → signature) is fully real and
 * verifiable; only the on-chain broadcast needs a live TON node, kept pluggable
 * via an optional `submit` hook (offline-safe, deterministic by default).
 *
 * TON address model
 * -----------------
 * A TON "raw" address is `<workchain>:<account_id>` where account_id is a
 * 256-bit hash. In a real wallet, account_id = sha256(StateInit cell). We use
 * account_id = sha256(pubkey) as a documented stand-in; the *encoding*
 * (tag/workchain/crc16/base64url) is byte-for-byte the real user-friendly
 * mainnet format, so produced addresses are 48-char base64url exactly like
 * Tonkeeper displays for mainnet (EQ…).
 *
...
export interface TonAddressOptions {
  ...
  /**
   * Mark as testnet-only address (tag | 0x80). Default FALSE on this mainnet
   * build — mainnet addresses do NOT carry the 0x80 test-only bit.
   */
...
export interface TonKeypair {
  ...
  /** User-friendly base64url address, 48 chars (bounceable mainnet by default). */
...
/**
 * Generate a fresh, cryptographically-random TON MAINNET keypair.
 * The address is a real 48-char base64url user-friendly mainnet address (EQ…).
 */
...
export function keypairFromSeed(
  ...
  // cell hash. The encoding below is the real user-friendly mainnet format.
  ...
  const address = encodeTonAddress(accountId, {
    ...
    testOnly: opts.testOnly ?? false, // MAINNET default: no 0x80 bit
    workchain,
  });
  return {
 ...
 * MAINNET default: testOnly=false → tag 0x11 (NOT 0x91). Yields EQ… addresses.
 */
export function encodeTonAddress(
  ...
  if (opts.testOnly ?? false) tag |= TAG_TEST_ONLY; // MAINNET default: no test bit
  const workchainByte =
...
export interface RealTonSignerConfig {
  ...
  /** Address options (bounceable / testOnly / workchain). Defaults to mainnet. */
  ...
  /** Network label for explorer URLs / settlement metadata. Default mainnet. */
...
export class RealTonSigner implements TonSigner {
  ...
  constructor(cfg: RealTonSignerConfig = {}) {
    let kp: TonKeypair;
    // Default to mainnet address flags unless explicitly overridden.
    const addrOpts: TonAddressOptions = cfg.addressOptions ?? {};
    if (cfg.seed) {
