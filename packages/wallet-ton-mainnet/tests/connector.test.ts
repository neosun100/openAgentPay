/**
 * Tests for @openagentpay/wallet-ton-mainnet — TON MAINNET Pay protocol +
 * wallet + crypto. Verifies mainnet address flags (tag 0x11, no 0x80 bit,
 * workchain 0, EQ… prefix) and real Ed25519 signing.
 */
...
// A deterministic real MAINNET keypair for assertions.
const KP = keypairFromSeed(new Uint8Array(32).fill(9));
const RECIPIENT = generateTonKeypair().address;

// ----------------------------------------------------------------------------
//  Keypair + MAINNET address generation
// ----------------------------------------------------------------------------

describe("TON MAINNET keypair + address", () => {
  it("generates a 48-char base64url mainnet address", () => {
    const kp = generateTonKeypair();
    expect(kp.address.length).toBe(48);
    expect(kp.address).toMatch(/^[A-Za-z0-9_-]{48}$/);
  });

  it("mainnet bounceable addresses start with EQ", () => {
    // tag 0x11 + workchain 0x00 always base64url-encodes to an "EQ…" prefix.
    for (let i = 1; i < 6; i++) {
      const kp = keypairFromSeed(new Uint8Array(32).fill(i));
      expect(kp.address.startsWith("EQ")).toBe(true);
    }
  });

  it("mainnet address decodes with NO test-only bit (tag 0x11)", () => {
    const dec = decodeTonAddress(KP.address);
    expect(dec.tag).toBe(0x11); // 0x11, NOT testnet's 0x91
  ...
  it("keypairFromHex round-trips the seed", () => {
    const a = keypairFromSeed(new Uint8Array(32).fill(5));
    const b = keypairFromHex(a.secretSeedHex);
...
describe("TON MAINNET address encode/decode", () => {
  it("encodes 32-byte account_id to 48 base64url chars (mainnet)", () => {
    const acct = new Uint8Array(32).fill(0xab);
    const addr = encodeTonAddress(acct); // mainnet default
    expect(addr.length).toBe(48);
    expect(isValidTonAddress(addr)).toBe(true);
    expect(decodeTonAddress(addr).testOnly).toBe(false);
  });

  it("decode recovers account_id, workchain, flags", () => {
    const acct = new Uint8Array(32).fill(0x42);
    const addr = encodeTonAddress(acct, { bounceable: true, workchain: 0 });
  ...
  it("explicit testOnly:true still sets the 0x80 bit (interop)", () => {
    const acct = new Uint8Array(32).fill(0x01);
    const addr = encodeTonAddress(acct, { testOnly: true });
    expect(decodeTonAddress(addr).testOnly).toBe(true);
    expect(decodeTonAddress(addr).tag).toBe(0x91);
  });
  ...
  it("rejects a CRC-tampered address", () => {
    ...
    const addr = encodeTonAddress(acct);
    const idx = 10;
...
describe("RealTonSigner (mainnet) — sign + verify", () => {
  it("defaults to a mainnet EQ… address", () => {
    const signer = new RealTonSigner({ seed: new Uint8Array(32).fill(3) });
    expect(signer.address.startsWith("EQ")).toBe(true);
    expect(decodeTonAddress(signer.address).testOnly).toBe(false);
  });
  ...
  it("offline path yields a deterministic mock txHash + mainnet explorer URL", async () => {
    const signer = new RealTonSigner({ seed: new Uint8Array(32).fill(4), network: "mainnet" });
    ...
    expect(r1.explorerUrl).toContain("tonviewer.com");
    expect(r1.explorerUrl).not.toContain("testnet.tonviewer.com");
  });
...
function makeConnector() {
  const signer = new RealTonSigner({ seed: new Uint8Array(32).fill(11), network: "mainnet" });
  const store = new MemoryInstrumentStore();
  return { signer, connector: new TonConnector({ signer, instrumentStore: store, network: "mainnet" }) };
}

describe("TonConnector — capabilities", () => {
  it("reports walletProvider=ton-mainnet + Ed25519 + jetton features", () => {
    const { connector } = makeConnector();
    const caps = connector.getCapabilities();
    expect(caps.walletProvider).toBe(WALLET_PROVIDER_ID);
    expect(caps.walletProvider).toBe("ton-mainnet");
  ...
  it("defaults to mainnet network in displayName", () => {
...
describe("TonConnector — createInstrument + getBalance", () => {
  it("creates instrument with signer.address as publicHandle (EQ…)", async () => {
    ...
    expect(inst.publicHandle.startsWith("EQ")).toBe(true);
  });
...
describe("TonConnector — signAuthorization + settle", () => {
  ...
  it("happy path → real signature + settle success (ton-mainnet)", async () => {
    const { signer, connector } = makeConnector();
    const inst = await connector.createInstrument({ userId: "alice" as UserId });
    ...
    expect(settled.network).toBe("ton-mainnet");
