/**
 * TON MAINNET wallet ↔ Conformance suite — proves TonConnector (mainnet)
 * backed by RealTonSigner passes the canonical 25-test WalletConnector contract.
 *
...
// A real, valid MAINNET recipient address derived in-process.
...
runWalletConformance(
  runner,
  {
    createConnector: () =>
      new TonConnector({
        signer: new RealTonSigner({ seed: TEST_SEED, network: "mainnet" }),
        instrumentStore: new MemoryInstrumentStore(),
        network: "mainnet",
      }),
    createUserId: (suffix) => `ton-mainnet-test-${suffix}` as UserId,
  ...
  },
  {
    ...
    skipSettle: false,
    suiteName: "wallet-ton-mainnet conformance (RealTonSigner)",
  }
);
