/**
 * RealMovementSigner — Ed25519 signer backed by @noble/curves, no Aptos/Movement SDK.
 * ============================================================================
 *
 * Movement is an Aptos-compatible Move-VM L2 (MoveVM execution + Aptos
 * framework). Its cryptographic identity is byte-for-byte identical to Aptos's
 * single-Ed25519 authentication key:
 *
 ...
 *
 * This matches what a Movement wallet / the Movement CLI display for a fresh
 * account (before any key rotation, account address === authentication key).
 *
 * Why not a Movement/Aptos SDK?
 ...
 *     via the optional `submit` hook so production can wire the Movement SDK's
...
import type { MovementSigner } from "./connector.js";

// Single-key Ed25519 authentication scheme identifier byte (Aptos/Move spec).
...
export interface MovementKeypair {
...
/**
 * Generate a fresh, cryptographically-random Movement keypair.
 * The address is a real sha3_256-derived authentication key, identical in
 * shape to what a Movement devnet faucet would fund.
 */
export function generateMovementKeypair(): MovementKeypair {
  const seed = ed25519.utils.randomPrivateKey(); // 32 bytes
  return keypairFromSeed(seed);
}

/** Derive the Movement authentication key / address from a 32-byte public key. */
export function authKeyFromPublicKey(pubkey: Uint8Array): string {
  if (pubkey.length !== 32) {
    throw new Error(`Movement pubkey must be 32 bytes, got ${pubkey.length}`);
  }
...
export function keypairFromSeed(seed: Uint8Array): MovementKeypair {
  if (seed.length !== 32) {
    throw new Error(`Movement seed must be 32 bytes, got ${seed.length}`);
  }
...
export function keypairFromPrivateKeyHex(privateKeyHex: string): MovementKeypair {
  const seed = hexToBytes(privateKeyHex);
  if (seed.length !== 32) {
    throw new Error(
      `Movement private key must be 32 bytes (64 hex chars), got ${seed.length}`
    );
  }
  return keypairFromSeed(seed);
}
...
export interface RealMovementSignerConfig {
  /** Hex private key ("0x" + 64 hex, or bare 64 hex). */
  readonly privateKeyHex?: string;
  /** Or supply a raw 32-byte seed directly. */
  readonly seed?: Uint8Array;
  /**
   * Optional balance reader — wired to a Movement fullnode REST endpoint in
  ...
  /**
   * Optional broadcast hook — wired to a Movement/Aptos SDK
...
export class RealMovementSigner implements MovementSigner {
  readonly address: string;
  readonly publicKeyHex: string;
  private readonly seed: Uint8Array;
  private readonly cfg: RealMovementSignerConfig;
  private readonly network: "mainnet" | "testnet" | "devnet";

  constructor(cfg: RealMovementSignerConfig = {}) {
    let kp: MovementKeypair;
    if (cfg.seed) {
      kp = keypairFromSeed(cfg.seed);
    } else if (cfg.privateKeyHex) {
      kp = keypairFromPrivateKeyHex(cfg.privateKeyHex);
    } else {
      kp = generateMovementKeypair();
    }
   ...
   * real Ed25519 signature over the canonical Movement transfer descriptor.
   ...
   * agent's cryptographic authorization, returned "0x"-hex (Move sig form).
   */
  async signAndSubmit(input: {
    ...
    const signature = "0x" + toHex(sigBytes); // "0x"-hex — matches Move sig form
  ...
  private explorerUrl(sig: string): string {
    return `https://explorer.movementnetwork.xyz/txn/${sig}?network=${this.network}`;
  }
}
...
/**
 * Deterministic, canonical string representation of a Movement coin transfer.
