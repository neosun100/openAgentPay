/**
 * Movement Pay Protocol Adapter + Wallet Connector
 * =================================================
 *
 * Movement is an Aptos-compatible Move-VM L2. Its account/crypto model is
 * byte-for-byte identical to Aptos, which lets us reuse the exact same proven
 * abstraction across yet another chain model:
 *
 ...
 *   - Native gas token: MOVE (8 decimals), running over the Aptos framework's
 *     `0x1::aptos_coin::AptosCoin` coin type.
 *
 * Still satisfies the same 5-method WalletConnector contract.
 *
 * Movement Pay protocol (OAP convention):
 *   - 402 / merchant returns a Movement Pay URL:
 *       movement:<recipient>?amount=...&coin=...&reference=...&label=...&message=...
 *   - Adapter parses URL → PaymentRequest
 *   - WalletConnector signs Ed25519 + submits via fullnode (or wallet adapter)
 *
 * Implementation strategy: PURE TypeScript (no Movement/Aptos SDK dependency)
...
export const PROTOCOL_ID = "movement-pay-v1" as ProtocolId;
export const WALLET_PROVIDER_ID = "movement" as WalletProviderId;
export const X_PAYMENT_MOVEMENT_HEADER = "X-PAYMENT-MOVEMENT";

/** Native MOVE coin type (Aptos framework coin, 8 decimals). */
export const MOVE_COIN_TYPE = "0x1::aptos_coin::AptosCoin";
/** Circle USDC on Movement testnet (6 decimals). */
const MOVEMENT_USDC_TESTNET =
  "0x447721a30109c662dde9c73a0c2c9c9c459fb5e5a9c92f03c50fa69737f5d08d::usdc::USDC";
/** Circle USDC on Movement mainnet (6 decimals). */
const MOVEMENT_USDC_MAINNET =
  "0xbae207659db88bea0cbead6da0ed00aac12edcdda169e591cd41c94180b46f3b::usdc::USDC";

// ============================================================================
//  Movement Pay URL parser
// ============================================================================

export interface MovementPayUrlFields {
...
const MOVEMENT_PAY_SCHEME = "movement:";

/** A Movement address is "0x" + 1..64 hex chars (we canonicalize to 64 elsewhere). */
function isLikelyMovementAddress(s: string): boolean {
  return /^0x[0-9a-fA-F]{1,64}$/.test(s);
}

/**
 * Parse a Movement Pay URL. Throws ProtocolError on malformed input.
 */
export function parseMovementPayUrl(url: string): MovementPayUrlFields {
  if (typeof url !== "string" || !url.startsWith(MOVEMENT_PAY_SCHEME)) {
    throw new ProtocolError(
      `Movement Pay URL must start with "${MOVEMENT_PAY_SCHEME}"`,
      "malformed"
    );
  }
  const afterScheme = url.slice(MOVEMENT_PAY_SCHEME.length);
  const queryIdx = afterScheme.indexOf("?");
  const recipient = queryIdx >= 0 ? afterScheme.slice(0, queryIdx) : afterScheme;
  if (!recipient) {
    throw new ProtocolError("Movement Pay URL missing recipient", "missing_field");
  }
  if (!isLikelyMovementAddress(recipient)) {
    throw new ProtocolError(
      `Movement Pay recipient is not a valid 0x address: ${recipient}`,
      "malformed"
    );
  }
  const fields: {
    -readonly [K in keyof MovementPayUrlFields]: MovementPayUrlFields[K];
  } = { recipient };
...
/** Build a Movement Pay URL from fields. Inverse of `parseMovementPayUrl`. */
export function buildMovementPayUrl(fields: MovementPayUrlFields): string {
  const params: string[] = [];
  ...
  return `${MOVEMENT_PAY_SCHEME}${fields.recipient}${params.length ? "?" + params.join("&") : ""}`;
}

// ============================================================================
//  Movement Pay ProtocolAdapter
// ============================================================================

export interface MovementPayAdapterConfig {
...
/**
 * MovementPayProtocolAdapter — recognizes 402 envelopes carrying a Movement Pay
 * URL and produces a wallet-agnostic PaymentRequest.
 *
 * Wire format we accept: { movementPay: "<URL>", ... } in body, raw string
 * body, or `x-movement-pay-url` header.
 */
export class MovementPayProtocolAdapter implements ProtocolAdapter {
  readonly id = PROTOCOL_ID;
  private readonly knownStableCoins: ReadonlySet<string>;
  private readonly now: () => number;

  constructor(cfg: MovementPayAdapterConfig = {}) {
    this.knownStableCoins = new Set(
      cfg.knownStableCoins ?? [MOVEMENT_USDC_TESTNET, MOVEMENT_USDC_MAINNET]
    );
  ...
  detect(response: HttpResponse402): boolean {
    ...
    return typeof url === "string" && url.startsWith(MOVEMENT_PAY_SCHEME);
  }
  ...
  async parsePaymentRequired(response: HttpResponse402): Promise<PaymentRequest> {
    ...
    if (!url) {
      throw new ProtocolError(
        "Movement Pay URL not found in body or header",
        "missing_field"
      );
    }
    const fields = parseMovementPayUrl(url);
    if (!fields.amount) {
      throw new ProtocolError("Movement Pay URL must specify ?amount=", "missing_field");
    }
    const isNative = !fields.coin || fields.coin === MOVE_COIN_TYPE;
    const isUsdc = fields.coin ? this.knownStableCoins.has(fields.coin) : false;
    const decimals = isNative ? 8 : isUsdc ? 6 : 8; // MOVE=8dp, USDC=6dp, default 8
    const currency = isNative ? "MOVE" : isUsdc ? "USDC" : "COIN";
    ...
    return {
      ...
      validBefore,
      nonce,
      rawPayload: { movementPayUrl: url, fields },
      ...(fields.message ? { description: fields.message } : {}),
    };
  }

  async buildRetry(signed: SignedAuthorization): Promise<HttpRetryEnvelope> {
    // Movement tx already broadcast (single-shot model) — attach the tx
    // signature for the merchant to verify on-chain.
    return {
      headers: {
        [X_PAYMENT_MOVEMENT_HEADER]: signed.signature,
      },
    };
  }
  ...
  private extractUrl(response: HttpResponse402): string | undefined {
    ...
    if (typeof body === "string" && body.startsWith(MOVEMENT_PAY_SCHEME)) return body;
    if (body && typeof body === "object") {
      const direct = (body as Record<string, unknown>)["movementPay"];
      if (typeof direct === "string") return direct;
      const url = (body as Record<string, unknown>)["url"];
      if (typeof url === "string" && url.startsWith(MOVEMENT_PAY_SCHEME)) return url;
    }
    const hdr = response.headers["x-movement-pay-url"];
...
// ============================================================================
//  Movement signer abstraction (pluggable)
// ============================================================================

export interface MovementSigner {
  /** "0x"-hex account address. */
  readonly address: string;
  /** "0x"-hex Ed25519 public key. */
  readonly publicKeyHex: string;
  /**
   * Sign + (optionally) submit a Movement coin transfer. Implementations:
   *   - DemoMovementSigner (this file) — fake signature for unit tests
   *   - RealMovementSigner (real-signer.ts) — real Ed25519, pluggable broadcast
   *   - Movement/Aptos SDK based signer (production)
   *   - Wallet adapter (Nightly / Razor / Leap browser wallet)
   */
  signAndSubmit(input: {
    readonly recipient: string;
 ...
 */
export class DemoMovementSigner implements MovementSigner {
  readonly address: string;
  readonly publicKeyHex: string;
  ...
  async signAndSubmit(input: {
    ...
    return {
      signature: sig,
      version: 1,
      explorerUrl: `https://explorer.movementnetwork.xyz/txn/${sig}?network=testnet`,
    };
  }
...
export interface MovementConnectorConfig {
  readonly signer: MovementSigner;
...
export class MovementConnector implements WalletConnector {
  private readonly signer: MovementSigner;
  ...
  constructor(cfg: MovementConnectorConfig) {
    this.signer = cfg.signer;
    this.store = cfg.instrumentStore;
    this.network = cfg.network ?? "testnet";
    this.defaultCoinType = cfg.defaultCoinType ?? MOVE_COIN_TYPE;
    this.now = cfg.now ?? Date.now;
  }
  ...
  async createInstrument(input: CreateInstrumentInput): Promise<Instrument> {
    ...
    const id = `payment-instrument-movement-${input.userId}` as InstrumentId;
  ...
  async getBalance(instrumentId: InstrumentId): Promise<Balance> {
    ...
    const isMove = this.defaultCoinType === MOVE_COIN_TYPE;
    return {
      instrumentId: inst.id,
      asset: {
        symbol: isMove ? "MOVE" : "USDC",
        decimals: isMove ? 8 : 6,
        ...(isMove ? {} : { contract: this.defaultCoinType }),
      },
      money: {
        amountAtomic: atomic.toString(),
        decimals: isMove ? 8 : 6,
        currency: isMove ? "MOVE" : "USDC",
      },
  ...
  /**
   * Movement transactions are single-step: the wallet builds + signs a Move
  ...
  async signAuthorization(
    ...
    if (input.request.protocol !== PROTOCOL_ID) {
      throw new Error(
        `MovementConnector only supports ${PROTOCOL_ID}, got ${input.request.protocol}`
      );
    }
    ...
    const coinType =
      ...
      (input.request.asset.symbol === "MOVE" ? MOVE_COIN_TYPE : this.defaultCoinType);
  ...
  async settle(signed: SignedAuthorization): Promise<SettlementResult> {
    // signAuthorization already produced the signed tx (Movement single-shot model).
    if (!signed.signature) {
      return {
        success: false,
        network: `movement-${this.network}`,
