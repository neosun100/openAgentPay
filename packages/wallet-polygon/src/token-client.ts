/**
 * EIP-3009 token client for Polygon Amoy — read balance + build/sign/submit
 * transferWithAuthorization.
 *
 * Mirrors `@openagentpay/wallet-hashkey`'s token-client. Same flow:
 ...
 *
 * Compatible with any ERC20 that implements EIP-3009 — here Circle's official
 * USDC on Polygon Amoy (chainId 80002).
 *
 * @license Apache-2.0
 */
...
import { polygonAmoyTestnet } from "./chain.js";
...
export interface Eip3009SignedAuthorization {
  ...
  readonly r: Hex;
  readonly s: Hex;
  /** The token's chainId (80002 for Polygon Amoy). */
  readonly chainId: number;
  /** The token contract address that the signature is bound to. */
  readonly verifyingContract: Address;
}

export interface PolygonAmoyTokenClientConfig {
  /** Token contract (Circle USDC on Polygon Amoy by default). */
  ...
  /** EIP-712 domain version override (Circle USDC uses "2"). */
 ...
 * transferWithAuthorization on Polygon Amoy.
 */
export class PolygonAmoyTokenClient {
  public readonly chain: typeof polygonAmoyTestnet;
  public readonly tokenAddress: Address;
  public readonly publicClient: PublicClient;
  private readonly domainVersion: string;

  constructor(config: PolygonAmoyTokenClientConfig) {
...
export function createWalletClientFromPrivateKey(
  privateKey: Hex,
  chain: typeof polygonAmoyTestnet,
  rpcUrl?: string
