/**
 * RealLitecoinSigner — secp256k1 (ECDSA) signer backed by @noble/curves, no bitcoinjs-lib.
 * ============================================================================
 *
 * Mirrors `RealBitcoinSigner` but for the Litecoin chain model. Litecoin is a
 * direct Bitcoin fork — same UTXO model, same secp256k1 curve, same hash160
 * witness program — the only material difference at the wallet layer is the
 * bech32 human-readable part (hrp):
 *
 *   - Crypto:      secp256k1 ECDSA (same curve as Bitcoin/Ethereum/Tron)
 *   - Address:     native SegWit P2WPKH — witness v0:
 *                  bech32(hrp, [0x00, ...convertbits(hash160(compressedPubkey),8,5)])
 *                  hash160(x) = ripemd160(sha256(x)).
 *                  testnet hrp = "tltc"  → "tltc1q…"
 *                  mainnet hrp = "ltc"   → "ltc1q…"
 *   - Asset:       LTC (8 dp, smallest unit = litoshi)
 ...
 * `submit` hook so production can wire a Litecoin Esplora/Electrum-LTC push
...
export type LitecoinNetwork = "testnet" | "mainnet";

/** Human-readable part (hrp) for the Litecoin bech32 SegWit address by network. */
export function hrpForNetwork(network: LitecoinNetwork): "tltc" | "ltc" {
  // Litecoin testnet uses "tltc"; mainnet uses "ltc" (BIP-173 SLIP-173).
  return network === "mainnet" ? "ltc" : "tltc";
}
...
export interface LitecoinKeypair {
  ...
  /** bech32 native-SegWit address — starts with "tltc1q" on testnet. */
...
export function keypairFromPrivateKey(
  ...
  if (priv.length !== 32) {
    throw new Error(`Litecoin private key must be 32 bytes, got ${priv.length}`);
  }
...
/**
 * Generate a fresh, cryptographically-random Litecoin keypair fully in-process.
 * The address is a real bech32 testnet P2WPKH address (starts "tltc1q"),
 * identical in shape to one a Litecoin testnet faucet would fund.
 */
export function generateLitecoinKeypair(
  network: LitecoinNetwork = "testnet"
): LitecoinKeypair {
...
export interface RealLitecoinSignerConfig {
  ...
  readonly network?: LitecoinNetwork;
  /**
   * Optional balance reader — wired to a Litecoin Esplora/Electrum-LTC REST
   * call in production. If omitted, getBalance() returns 0 (offline-safe
   * default). Returns the confirmed balance in litoshis.
   */
  ...
  readonly submit?: (input: {
    ...
    readonly amountLitoshis: string;
...
export interface LitecoinSignResult {
  /** DER-encoded ECDSA signature (hex, no 0x) — the canonical Bitcoin/LTC sig form. */
  readonly signature: string;
...
export class RealLitecoinSigner {
  readonly address: string;
  readonly network: LitecoinNetwork;
  private readonly priv: Uint8Array;
  private readonly cfg: RealLitecoinSignerConfig;

  constructor(cfg: RealLitecoinSignerConfig = {}) {
    this.network = cfg.network ?? "testnet";
    let kp: LitecoinKeypair;
    if (cfg.privateKey) {
      ...
      kp = generateLitecoinKeypair(this.network);
    }
  ...
  /**
   * Sign a canonical Litecoin transfer descriptor with secp256k1 ECDSA. The
   * signed message is the BIP-143-style sighash = dSHA256(descriptor) — the
   * same double-SHA256 commitment Bitcoin/Litecoin use for transaction
   * signing. The signature is REAL and verifiable offline (DER-encoded, low-S
   * normalized); the `submit` hook (when present) assembles + broadcasts the
   * actual PSBT.
   */
  async signAndSubmit(input: {
    ...
    // lowS:true → canonical secp256k1 signatures (BIP-62 / BIP-146).
    const sig = secp256k1.sign(sighash, this.priv, { lowS: true });
    const signature = toHex(sig.toDERRawBytes());
    // Litecoin (like Bitcoin) displays txids big-endian (reverse of internal byte order).
  ...
  private explorerUrl(txid: string): string {
    // Litecoin block explorers. blockchair supports both networks.
    if (this.network === "mainnet") {
      return `https://blockchair.com/litecoin/transaction/${txid}`;
    }
    return `https://blockchair.com/litecoin/testnet/transaction/${txid}`;
  }
}
...
/**
 * Deterministic, canonical string representation of a Litecoin transfer. Stable
...
export function canonicalTransferDescriptor(fields: {
  ...
  const parts = [
    ...
    `from=${fields.from}`,
    `to=${fields.to}`,
    `amount_litoshis=${fields.amountLitoshis}`,
...
/** Litecoin's double-SHA256: sha256(sha256(x)) — same as Bitcoin. */
