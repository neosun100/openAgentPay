/**
 * xsAI plugin tests — verify the descriptor shape xsai's tool() expects.
 */

import { describe, expect, it } from "vitest";
import { createXsaiPaymentTool } from "../src/index.js";
import type {
  PaymentManager,
  Session,
  SessionId,
  ...
  return {
    ...
    async processPayment(input) {
      ...
      return {
        ...
        settlement: {
          success: true,
          transactionRef: "0xXSAI_TX" as any,
          network: "mock",
...
describe("createXsaiPaymentTool", () => {
  it("returns xsAI-shaped descriptor", () => {
    const tool = createXsaiPaymentTool({
  ...
  it("execute(input) delegates to runPayment", async () => {
    const tool = createXsaiPaymentTool({
      manager: makeMockManager(),
      userId: "alice" as UserId,
      defaultWalletProvider: "test-wallet" as WalletProviderId,
    ...
    expect(r.txHash).toBe("0xXSAI_TX");
  ...
  it("respects walletProvider override in input", async () => {
    const tool = createXsaiPaymentTool({
      manager: makeMockManager(),
      userId: "alice" as UserId,
      defaultWalletProvider: "test-wallet" as WalletProviderId,
